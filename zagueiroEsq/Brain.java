...
class Brain extends Thread implements SensorInput
{
    ...
    public void run(){
		...
		while( !m_timeOver){
			if(m_memory.isEmpty()){
				...
				if (m_side == 'l')
					object = m_memory.getObject("flag p l b");
				else
					object = m_memory.getObject("flag p r t");
				...
				if(!posicao)
				{
					...
					{
						System.out.println("Eu sou o zagueiro 2 e cheguei no meu destino");
							posicao = true;
					}
							}
				else
				{
					...
					else if(ball != null && ball.m_distance < 25 && ball.m_distance > 0.7)
					{
					...
					else if (ball != null && ball.m_distance <= 1.0)
					{
					    ...
					    {
					    	m_agent.kick(flag.m_distance + 40, flag.m_direction);
							m_memory.clearInfo();
					    	chutei++;
					    }
					    else
    ...
    {
    	m_agent.move(-0.7*52.5, 34-0.35*68.0);
    }
