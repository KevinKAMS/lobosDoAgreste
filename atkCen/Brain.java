...
class Brain extends Thread implements SensorInput
{
    ...
    public void run(){
		...
		while( !m_timeOver){
			if(m_memory.isEmpty()){
				...
				if (m_side == 'l')
					object = m_memory.getObject("flag p r c");
				else
					object = m_memory.getObject("flag p l c"); 
    ...
    public void entrarEmPosicao(){
		if (object == null)
		{
			...
			System.out.println("Eu sou o atacante 2 e cheguei no meu destino");
			posicao = true;
		}
    }
    ...
    public void rotina(){
		...
		else if(ball != null && ball.m_distance <= 1.0)
		{
			...
			m_agent.kick(4*goal.m_distance, goal.m_direction);
		}
		else
		{
			posicao = false;
    ...
    {
    	m_agent.move(0*52.5, 34-0.7*68.0);
    }
