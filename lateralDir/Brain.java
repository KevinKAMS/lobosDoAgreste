...
class Brain extends Thread implements SensorInput
{
    ...
    public void run(){
		...
		while( !m_timeOver){
			if(m_memory.isEmpty()){
				...
				object = m_memory.getObject("flag c t");
				if (m_side == 'l')
				...
				if(!posicao)
				{
					if (object == null)
					{
						...
						System.out.println("Eu sou o Meia 2 e cheguei no meu destino");
						posicao = true;
					}
				}
				else
				{
					...
					else if(ball != null && ball.m_distance <= 1.0)
					{
					    ...
					    {
					    	m_agent.kick(flagTop.m_distance + 40, flagTop.m_direction);
							m_memory.clearInfo();
					    	chutei++;
					    }
					    else
    ...
    {
    	m_agent.move(-0.1*52.5, 34-0.8*68.0);
    }
